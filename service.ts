...
io.on('connection', (socket) => {
    ...
    socket.on('cmd-connect', async (data) => {
        if (client && (data?.force || data?.restart)) {
            console.log('[SYSTEM] Force Restart Requested');
            try {
                await client.destroy();
                console.log('[SYSTEM] Client destroyed for restart');
            } catch (e) {
                console.error('[WHATSAPP] Destroy Error during restart:', e.message);
            }
            client = null;
        }
