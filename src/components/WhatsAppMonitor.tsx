...
const WhatsAppMonitor: React.FC = () => {
  ...
  const handleConnect = () => { socket.emit('cmd-connect', { force: isScan }); setStatus('LAUNCHING'); };
  const handleDisconnect = () => { if(confirm("Putuskan sambungan WhatsApp?")) socket.emit('cmd-disconnect'); };
  const handleRefreshQR = () => { 
    if(confirm("Segarkan kod QR? Sambungan sedia ada akan diputuskan.")) {
        socket.emit('cmd-disconnect');
        setTimeout(() => {
            socket.emit('cmd-connect');
            addLog('New QR Code Requested...');
        }, 1500);
    }
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-40px)] bg-darker -m-4 md:-m-6 lg:-m-8 overflow-hidden font-sans">
      ...
      {activeTab === 'connection' && (
          <div className="flex-1 overflow-auto bg-slate-100 flex items-start md:items-center justify-center p-2 lg:p-8">
              <div className="bg-white w-full max-w-5xl min-h-fit md:h-[600px] rounded-3xl md:rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row relative my-2">
                  
                  {/* LEFT SIDE: QR / STATUS VISUAL */}
                  <div className="md:w-1/2 bg-slate-900 p-6 md:p-8 flex flex-col justify-center items-center relative overflow-hidden text-white shrink-0 min-h-[300px]">
                      ...
                      <div className="relative z-10 flex flex-col items-center">
                          {isConnected ? (
                              <div className="w-40 h-40 md:w-48 md:h-48 bg-emerald-500/20 rounded-[2rem] flex items-center justify-center border-4 border-emerald-500 animate-pulse">
                                  <CheckCircle size={60} className="text-emerald-400 drop-shadow-lg" />
                              </div>
                          ) : isScan && qrCode ? (
                              ...
                              <div className="w-40 h-40 md:w-48 md:h-48 bg-slate-800 rounded-[2rem] flex items-center justify-center border-4 border-slate-700">
                                  {isReconnecting ? <Loader2 size={50} className="text-slate-600 animate-spin" /> : <ScanLine size={50} className="text-slate-600" />}
                              </div>
                          )}
                  ...
                  <div className="md:w-1/2 p-6 md:p-10 flex flex-col justify-between overflow-y-auto">
                      ...
                      <div className="space-y-4 my-6">
                          ...
                          {/* REFRESH QR / FORCE SYNC SECTION */}
                          <div className="flex gap-2">
                              <button onClick={handleRefreshQR} className="flex-1 bg-cyan-50 text-cyan-600 py-2 rounded-xl font-bold uppercase tracking-widest text-[8px] hover:bg-cyan-100 transition-all flex items-center justify-center gap-2 border border-cyan-100 shadow-sm active:scale-95">
                                  <RefreshCw size={10} className="animate-spin-slow" /> Refresh QR Baru
                              </button>
                               <button onClick={() => { socket.emit('cmd-refresh-profile'); alert('Permintaan sinkronasi profil dihantar...'); }} className="flex-1 bg-emerald-50 text-emerald-600 py-2 rounded-xl font-bold uppercase tracking-widest text-[8px] hover:bg-emerald-100 transition-all flex items-center justify-center gap-2 border border-emerald-100 shadow-sm active:scale-95">
                                  <User size={10} /> Sync Profil Admin
                              </button>
                          </div>
                      </div>
