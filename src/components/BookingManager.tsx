...
const BookingActions = ({ booking, onComplete, onInvoice, onDelete, showToast }: any) => (
    <>
        <button 
        ...
        <button onClick={() => onDelete(booking.id)} className="p-2 hover:bg-red-500/10 rounded-lg text-slate-400 hover:text-red-500">
            <Trash2 size={14} />
        </button>
    </>
);
          ...
          <div className="glass-panel overflow-hidden">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                    ...
                    <tr key={booking.id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                           <BookingActions booking={booking} onComplete={handleMarkCompleted} onInvoice={handleConvertToInvoice} onDelete={handleDeleteBooking} showToast={showToast} />
                        </div>
                      </td>
                    </tr>
                  )})}
                  {bookings.filter(b => (b.booking_date || b.date) === selectedDate).length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-slate-500 text-sm italic font-bold uppercase tracking-widest">
                        Tiada tempahan untuk tarikh ini
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <div className="md:hidden divide-y divide-white/5">
                {bookings.filter(b => {
                    ...
                    <div className="px-6 py-12 text-center text-slate-500 text-[10px] font-bold uppercase tracking-widest">
                       Tiada tempahan untuk tarikh ini
                    </div>
                ) : bookings.filter(b => {
                    const bDate = (b.booking_date || b.date || '').toString().split('T')[0];
                    ...
                    <div key={booking.id} className="p-4 space-y-3">
                        <div className="flex justify-between items-start">
                            <div className="min-w-0">
                                <h4 className="text-xs font-black text-white uppercase truncate pr-4">{booking.customer_name || booking.customerName}</h4>
                                <p className="text-[10px] text-secondary font-bold">{booking.customer_phone || booking.phone}</p>
                            </div>
                            <div className={`px-2 py-0.5 rounded shrink-0 text-[8px] font-black uppercase ${booking.status === 'Confirmed' ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20'}`}>
                                {booking.status}
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                             <div className="bg-white/5 rounded-xl p-2 border border-white/5">
                                 <p className="text-[8px] text-slate-500 uppercase font-black tracking-widest mb-1">Team & Slot</p>
                                 <p className="text-[10px] text-white font-bold truncate">{booking.team_name || booking.team}</p>
                                 <p className="text-[9px] text-primary font-black uppercase tracking-tight">{booking.slot_label || booking.timeSlot}</p>
                             </div>
                             <div className="bg-white/5 rounded-xl p-2 border border-white/5">
                                 <p className="text-[8px] text-slate-500 uppercase font-black tracking-widest mb-1">Servis</p>
                                 <p className="text-[10px] text-white font-bold truncate">{booking.service_type || booking.serviceType}</p>
                                 <p className="text-[9px] text-slate-400 font-bold truncate">{booking.unit_type || booking.unitType}</p>
                             </div>
                        </div>
